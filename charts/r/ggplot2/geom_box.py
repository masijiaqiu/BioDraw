...
def geom_box(data, parameters, output):
    # parameters
    ## aes's parameters
    aes_param = ''
    aes = None
...
#        os.system('echo "postscript(\''+output+'\')" >>'+r_tmp_file)
        ...
        os.system('echo "image = p+geom_boxplot('+geom_param+')" >> '+r_tmp_file)
...
def geom_transbox(data, parameters, output):
    # parameters
    ## aes's parameters
    aes_param = ''
    aes = None
...
#        os.system('echo "postscript(\''+output+'\')" >>'+r_tmp_file)
        os.system('echo "p = ggplot(input_data'+aes_param+')" >> '+r_tmp_file)
        os.system('echo "image = p+geom_boxplot('+geom_param+')+ coord_flip()" >> '+r_tmp_file)
        os.system('echo "ggsave(file=\''+output+'\', plot=image)" >> '+r_tmp_file)
#        os.system('echo "dev.off()" >>'+r_tmp_file)
        os.system(rscript_cmd+' '+r_tmp_file)
    finally:
