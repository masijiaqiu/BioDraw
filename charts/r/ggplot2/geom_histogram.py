...
def geom_histogram(data, parameters, output):
    # parameters
    ## aes's parameters
    aes_param = ''
    aes = None
    ...
    if 'bins' in parameters.keys():
        geom_param = geom_param + ',bins=\'' + str(parameters['bins'])+'\''
...
#        os.system('echo "postscript(\''+output+'\')" >>'+r_tmp_file)
        ...
        os.system('echo "image = p+geom_histogram('+geom_param+')" >> '+r_tmp_file)
...
def geom_freqpoly(data, parameters, output):
    # parameters
    ## aes's parameters
    aes_param = ''
    aes = None
    ...
    if aes is not None:
        ...
        if 'y' in aes.keys():
            aes_param_tmp = aes_param_tmp + ',y=' + str(aes['y'])
        if 'colour' in aes.keys():
            aes_param_tmp = aes_param_tmp + ',colour=' + str(aes['colour'])
        if aes_param_tmp.strip().startswith(','):
            aes_param_tmp = aes_param_tmp[1:]
    ...
    if 'method' in parameters.keys():
        geom_param = geom_param + ',method=\'' + str(parameters['method'])+'\''
    if 'binaxis' in parameters.keys():
        geom_param = geom_param + ',colour=\'' + str(parameters['colour'])+'\''
    if geom_param.strip().startswith(','):
        geom_param = geom_param[1:]
...
#        os.system('echo "postscript(\''+output+'\')" >>'+r_tmp_file)
        os.system('echo "p = ggplot(input_data'+aes_param+')" >> '+r_tmp_file)
        os.system('echo "image = p+geom_freqpoly('+geom_param+')" >> '+r_tmp_file)
        os.system('echo "ggsave(file=\''+output+'\', plot=image)" >> '+r_tmp_file)
#        os.system('echo "dev.off()" >>'+r_tmp_file)
        os.system(rscript_cmd+' '+r_tmp_file)
    finally:
